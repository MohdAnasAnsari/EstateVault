import 'dotenv/config';
import Fastify from 'fastify';
import helmet from '@fastify/helmet';
import cors from '@fastify/cors';
import rateLimit from '@fastify/rate-limit';
import jwt from '@fastify/jwt';
import cookie from '@fastify/cookie';
import { createFastifyLogger, createLogger } from '@vault/logger';
import { IORedis } from '@vault/cache';

import { analyticsRoutes } from './routes/analytics.js';
import { marketIntelligenceRoutes } from './routes/market-intelligence.js';
import { currencyRoutes } from './routes/currency.js';
import { adminRoutes } from './routes/admin.js';

const logger = createLogger('analytics-service');

const PORT = parseInt(process.env['PORT'] ?? '3008', 10);
const HOST = process.env['HOST'] ?? '0.0.0.0';
const REDIS_URL = process.env['REDIS_URL'] ?? 'redis://localhost:6379';
const CHANNEL_PREFIX = process.env['REDIS_EVENT_CHANNEL_PREFIX'] ?? 'vault:';

// ─── Analytics counters ───────────────────────────────────────────────────────

interface NdaSignedPayload {
  dealRoomId?: string;
  ndaId?: string;
}

interface OfferSubmittedPayload {
  dealRoomId?: string;
  offerId?: string;
  amount?: number;
  currency?: string;
}

async function handleNdaSigned(payload: NdaSignedPayload): Promise<void> {
  // Update analytics counters — e.g., daily NDA counter in Redis
  logger.info({ dealRoomId: payload.dealRoomId }, 'NDA signed event received');
}

async function handleOfferSubmitted(payload: OfferSubmittedPayload): Promise<void> {
  logger.info({ dealRoomId: payload.dealRoomId, amount: payload.amount }, 'Offer submitted event received');
}

// ─── Redis event subscriptions ────────────────────────────────────────────────

function subscribeToEvents(): void {
  const RedisClass = IORedis as unknown as new (
    url: string,
    opts: { maxRetriesPerRequest: number | null; enableReadyCheck: boolean; lazyConnect: boolean },
  ) => {
    subscribe(...channels: string[]): Promise<number>;
    on(event: string, listener: (channel: string, message: string) => void): void;
    on(event: 'error', listener: (err: Error) => void): void;
  };

  const sub = new RedisClass(REDIS_URL, {
    maxRetriesPerRequest: null,
    enableReadyCheck: false,
    lazyConnect: true,
  });

  sub.on('error', (err) => {
    logger.error({ err }, 'Redis subscriber error');
  });

  const channels = [
    `${CHANNEL_PREFIX}nda.signed`,
    `${CHANNEL_PREFIX}offer.submitted`,
  ];

  sub.subscribe(...channels).then(() => {
    logger.info({ channels }, 'Subscribed to Redis event channels');
  }).catch((err: unknown) => {
    logger.error({ err }, 'Failed to subscribe to Redis channels');
  });

  sub.on('message', (channel, message) => {
    try {
      const payload = JSON.parse(message) as Record<string, unknown>;

      if (channel === `${CHANNEL_PREFIX}nda.signed`) {
        handleNdaSigned(payload as NdaSignedPayload).catch((err: unknown) =>
          logger.error({ err }, 'Error handling nda.signed event'),
        );
      }

      if (channel === `${CHANNEL_PREFIX}offer.submitted`) {
        handleOfferSubmitted(payload as OfferSubmittedPayload).catch((err: unknown) =>
          logger.error({ err }, 'Error handling offer.submitted event'),
        );
      }
    } catch (err) {
      logger.error({ err, channel }, 'Failed to process Redis event');
    }
  });
}

// ─── App factory ──────────────────────────────────────────────────────────────

async function buildApp() {
  const app = Fastify({
    logger: createFastifyLogger('analytics-service') as Parameters<typeof Fastify>[0]['logger'],
    trustProxy: true,
    disableRequestLogging: false,
  });

  // ── Security plugins ───────────────────────────────────────────────────────
  await app.register(helmet, {
    contentSecurityPolicy: false,
  });

  await app.register(cors, {
    origin: process.env['ALLOWED_ORIGINS']?.split(',') ?? ['*'],
    credentials: true,
  });

  await app.register(rateLimit, {
    max: 100,
    timeWindow: '1 minute',
  });

  await app.register(jwt, {
    secret: process.env['NEXTAUTH_SECRET'] ?? 'change-me',
  });

  await app.register(cookie, {
    secret: process.env['NEXTAUTH_SECRET'] ?? 'change-me',
  });

  // ── Health endpoint ────────────────────────────────────────────────────────
  app.get('/health', async (_request, reply) => {
    return reply.send({
      status: 'ok',
      service: 'analytics-service',
      timestamp: new Date().toISOString(),
    });
  });

  // ── Routes ─────────────────────────────────────────────────────────────────
  await app.register(analyticsRoutes, { prefix: '/analytics' });
  await app.register(marketIntelligenceRoutes, { prefix: '/market-intelligence' });
  await app.register(currencyRoutes, { prefix: '/currency' });
  await app.register(adminRoutes, { prefix: '/admin' });

  // ── Error handler ──────────────────────────────────────────────────────────
  app.setErrorHandler((error, _request, reply) => {
    logger.error({ err: error }, 'Unhandled error');
    const statusCode = error.statusCode ?? 500;
    reply.code(statusCode).send({
      success: false,
      error: { code: 'INTERNAL_ERROR', message: error.message },
    });
  });

  return app;
}

// ─── Main ─────────────────────────────────────────────────────────────────────

async function main(): Promise<void> {
  try {
    // Subscribe to Redis events
    subscribeToEvents();

    // Build and start app
    const app = await buildApp();
    await app.listen({ port: PORT, host: HOST });

    logger.info({ port: PORT, host: HOST }, 'analytics-service started');
  } catch (err) {
    logger.error({ err }, 'Failed to start analytics-service');
    process.exit(1);
  }
}

main();
