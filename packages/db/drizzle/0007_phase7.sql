...
CREATE INDEX IF NOT EXISTS "users_tier_active_idx"
  ON "users" ("access_tier", "last_active_at");

-- Phase 7 composite indexes: listings
CREATE INDEX IF NOT EXISTS "listings_status_visibility_city_idx"
  ON "listings" ("status", "visibility", "city");

CREATE INDEX IF NOT EXISTS "listings_status_type_price_idx"
  ON "listings" ("status", "asset_type", "price_amount");

CREATE INDEX IF NOT EXISTS "listings_status_quality_created_idx"
  ON "listings" ("status", "quality_tier", "created_at");

CREATE INDEX IF NOT EXISTS "listings_fraud_verif_idx"
  ON "listings" ("ai_fraud_flag", "verification_status");

-- Phase 7 composite indexes: deal_rooms
CREATE INDEX IF NOT EXISTS "deal_rooms_buyer_status_idx"
  ON "deal_rooms" ("buyer_id", "status");

CREATE INDEX IF NOT EXISTS "deal_rooms_seller_lastmsg_idx"
